...
When goals conflict, prioritize correctness, then clarity, simplicity, maintainability, readability, and finally micro-optimization. Prefer consistency over novelty and clarity over cleverness.

## Communication

- Be extremely concise. Sacrifice grammar for concision when needed, never clarity. Lead with the outcome and include the evidence or explanation needed to assess it; assume more detail will be requested separately.
- Be candid and avoid flattery. Prefer correctness over agreement. Challenge assumptions or suggest alternatives when they materially affect the result.
...
- Make the smallest correct change that fits the codebase. Keep changes focused and reviewable. Preserve unrelated behavior and the user's existing edits; do not overwrite or revert work outside the task.
...
- Keep data-access layers thin: transport and mapping. Make orchestration of multiple calls, caching, and refresh behavior intentional.
...
- Introduce dependencies, configuration options, or indirection only for a concrete need. Avoid premature generalization and leaky abstractions; keep shared utilities focused rather than general dumping grounds.
- Remove code made obsolete by the change. Keep unrelated cleanup separate.
- Use comments to explain intent, constraints, or non-obvious caveats, not what names already convey. TODO comments are fine for clearly out-of-scope follow-up. Keep affected comments and documentation accurate.

## UI and types, when applicable

- Reuse the project's components and design patterns. Give components clear responsibilities, explicit props, and readable rendering logic. Split components when it improves readability, testing, or reuse, not for cosmetic reasons.
...
- Prefer explicit, understandable types. Distinguish domain data from view models where useful; use type escape hatches or complex generics only with a concrete justification.
...
- Return analysis and plans in chat unless a file update is requested. Use the project's existing source of truth and formats.
...
- When committing is requested, inspect the diff and follow the repository's message conventions. Keep each commit to one logical change and split unrelated concerns. Use a short subject; add a body explaining what and why only for complex changes, wrapped at about 72 characters.
- Never add Co-Authored-By or other attribution trailers unless explicitly requested.
- After code-affecting changes, end the response with a one-line commit message proposal without committing.
